...
void sdramphy_check_status(FILE *fp)
{
	...
	jeq_code(fp, PHY_STS_BASE + 0x50, GENMASK(15, 0), 0,
		 "l_sdramphy_train");

	/* retrain if PHY_STS050[31:16] == 0 */
	jeq_code(fp, PHY_STS_BASE + 0x50, GENMASK(31, 16), 0,
		 "l_sdramphy_train");
#endif
}
...
#if defined(CONFIG_FPGA_ASPEED) || defined(CONFIG_ASPEED_PALLADIUM)
	wr_single(fp, MMC_BASE + 0x0c,
		  BIT(7) | BIT(5) | BIT(0) | (0x5d << 8) | (0x4000 << 16));
#else
	wr_single(fp, MMC_BASE + 0x0c,
		  BIT(7) | BIT(5) | BIT(0) | (0x5f << 8) | (0x42aa << 16));
#endif
...
#define var_win					(SRAM1_BASE + 0)
	...
	jeq_code(fp, SEARCH_RDWIN_ANCHOR_0, GENMASK(31, 0), SEARCH_RDWIN_PTRN_0,
		 "l_check_value_start");
	jeq_code(fp, SEARCH_RDWIN_ANCHOR_1, GENMASK(31, 0), SEARCH_RDWIN_PTRN_1,
		 "l_check_value_start");
